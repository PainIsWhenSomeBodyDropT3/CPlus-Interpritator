...
namespace IT
{
	enum IDDATATYPE
	{
		NONE = 1, INT, STR					//типы данных идентификаторов
	};
	enum IDTYPE
	{
	...
	struct Entry
	{
		...
		char id[TI_STR_MAXSIZE];		//идентификатор(автоматически усекаетс€ до ID_MAXSIZE)
	...
	int IsId(IdTable&, char id[ID_MAXSIZE], bool);		//возврат: номер строки(если есть), TI_NULLIDX(если нет)(idtable, id)
